...
const GaugeChart = dynamic(() => import("@/components/charts/gauge/GaugeChart"), {
  ssr: false,
});
...
const getLatestEvent = () => {
  ...
  const moisture = randInt(20, 90);
  const soilPH = +(Math.random() * (8 - 6) + 6).toFixed(1);
  const irrigation = Math.random() > 0.5 ? "On" : "Off";

  // Dynamic message
  let message = "🌱 Conditions are optimal";
  if (moisture < 40) message = "💧 Water needed soon – low soil moisture";
  else if (temperature > 35) message = "🔥 High temperature alert – protect crops";
  else if (humidity < 40) message = "💨 Low humidity – monitor irrigation";
  else if (soilPH < 6 || soilPH > 7.5) message = "🧪 Soil pH out of optimal range";
  else if (irrigation === "On") message = "💧 Irrigation is running";
    ...
    <div className="space-y-6">
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-4 mt-4">
          ...
          <GaugeChart
            title="Soil pH"
            value={latestEvent.soilPH}
            unit=""
            min={0}
            max={14}
            thresholds={{ min: 6.5, max: 7.5 }}
          />
          <GaugeChart
            title="Irrigation"
            value={latestEvent.irrigation}
          />
        </div>
      )}
